...
<TextInput 
  editable={false}
  style={{ borderColor: tokens.colors.gray[300] }}
/>
```
...
```tsx
<Text style={{ color: tokens.colors.gray[500], fontSize: 13 }}>
...
<View style={{ 
  ...
  <Text style={{ color: tokens.colors.warning.main }}>
    Перерыв более 15 минут
  </Text>
</View>
```
...
<View style={{ 
  ...
  <MaterialCommunityIcons 
    ...
    size={20} 
  />
  <Text style={{ color: tokens.colors.info.dark }}>
    Смена начинается в 09:00
  </Text>
</View>
```
...
```tsx
<Text style={{ color: tokens.colors.text.primary, fontSize: 16 }}>
  Заголовок секции
</Text>
```
...
```tsx
<Text style={{ color: tokens.colors.text.secondary, fontSize: 13 }}>
  Обновлено 5 минут назад
</Text>
```
...
```tsx
<Text style={{ color: tokens.colors.text.disabled }}>
  Недоступно
</Text>
```
