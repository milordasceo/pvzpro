...
<View style={{ paddingHorizontal: screenPadding }}>{/* Контент */}</View>;
```
...
>
  ...
  <Card style={{ padding: tokens.spacing.lg }}>
    ...
    <View style={{ gap: tokens.spacing.sm }}>{/* Список сотрудников */}</View>
  </Card>
</ScrollView>
```
...
<Dialog visible={visible} onDismiss={onClose}>
  ...
  >
    <Body>Вы уверены, что хотите удалить этот элемент?</Body>

    <Caption>Это действие нельзя отменить</Caption>
  </View>
