...
<SearchInput value={query} onChangeText={setQuery} placeholder="Поиск..." />;
```
...
<ErrorState message="Не удалось загрузить данные" onRetry={handleRetry} />;
```
...
<MetaRow icon="calendar" label="Дата" rightValue="20.10.2025" />;
```
...
<Searchbar placeholder="Поиск" onChangeText={setSearchQuery} value={searchQuery} />;
...
<SearchInput value={searchQuery} onChangeText={setSearchQuery} placeholder="Поиск" />;
```
